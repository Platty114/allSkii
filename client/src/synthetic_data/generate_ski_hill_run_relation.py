...
for index, hill in ski_hills_df.iterrows():
    ...
    # Generate a 5km box around the ski hill (10km square centered on the hill)
    buffer_in_deg = 10 * KM_TO_DEG  # Convert 5km buffer to degrees approximately
    ...
    for run_index, run in ski_runs_df.iterrows():
        ...
        # Check if the ski run intersects the box
        if hill_box.intersects(run['geometry']):
            print(hill, run, hill_box.intersects(run['geometry']))
